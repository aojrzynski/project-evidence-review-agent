...
8. **PR #8 human-readable project evidence report** — completed capability: after claim review and follow-up validation pass, write `project_evidence_report.md` as the first artifact to open in a successful full run. The report assembles validated artifacts, separates supported points, unclear points, gap signals, possible tensions, human checks, limitations, and authority boundaries, and does not make another LLM call.
9. **PR #9 optional LangGraph orchestration** — current capability: add `--orchestrator standard|langgraph`, keep standard Python orchestration as the default, and use LangGraph only as an optional adapter whose nodes call shared workflow stages rather than owning business logic.
...
## Current PR #9 boundary

PR #9 preserves the PR #8 evidence and review boundary while adding optional orchestration metadata. Standard mode remains the default. LangGraph mode, when installed and requested, coordinates the same stages and does not add evidence semantics, LLM calls, validation rules, report decisions, LangSmith tracing, or deployment requirements.

The workflow records the supplied review question, retrieves lexically relevant chunks from the deterministic evidence index, writes a bounded JSON evidence pack, renders `evidence_pack.md`, builds `llm_safe_review_context.json`, writes validated `claim_review.json`, writes `missing_evidence.json` and `contradiction_log.json` only when claim review succeeds, and then writes `project_evidence_report.md` only when all prior LLM and follow-up validation passes.
